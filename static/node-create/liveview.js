var arrayColor = ["#4CAF50", "#1e4695", "#b3144c", "#cd4e14"
...
class Drawflow {
    ...
    click(e) {
        ...
        this.dispatch("nodes", {node: this.ele_selected, e: e})
    ...
    drawConnection(ele) {
        ...
        let trt = this.ele_selected.classList[1].match(/\d+/)[0];

        path.classList.add("main-path" + (trt - 1));
    ...
    addNode(name, num_in, num_out, ele_pos_x, ele_pos_y, classoverride, data, html, typenode = false) {
        ...
        for (var x = 0; x < num_out; x++) {
            ...
            span.innerHTML = "<b>" + (x + 1) + "</b>"
            output.appendChild(span);
            output.classList.add("output");
            output.style.backgroundColor = arrayColor[x];
        ...
        Object.entries(data).forEach(function (key, value) {
            if (typeof key[1] === "object") {
                ...
                for (var i = 0; i < elems.length; i++) {
                    if ("LABEL" === elems[i].tagName) {
                        elems[i].textContent = key[1];
                    } else {
                        elems[i].value = key[1];
                    }
                }
    ...
    addNodeImport(dataNode, precanvas) {
        ...
        Object.keys(dataNode.inputs).map(function (input_item, index) {
            ...
            Object.keys(dataNode.inputs[input_item].connections).map(function (output_item, index) {
                ...
                path.classList.add("main-path" + (index));
                path.classList.add("animation-line");
        ...
        this.dispatch('nodeCreatedImport', dataNode.id);

    }

     insertObjectkeyss(object, name, completname) {
        if (object === null) {
            var object = dataNode.data[name];
        } else {
            var object = object[name]
        ...
        Object.entries(object).forEach(function (key, value) {
            if (typeof key[1] === "object") {
              this.insertObjectkeyss(object, key[0], name + '-' + key[0]);
            } else {
                var elems = content.querySelectorAll('[df-' + completname + '-' + key[0] + ']');
                for (var i = 0; i < elems.length; i++) {
                    if ("LABEL" === elems[i].tagName) {
    ...
    refresh() {
        const v = JSON.parse(JSON.stringify(this.drawflow));
        this.clear();
        this.drawflow = v
        this.load();
    ...
    getNode() {
        ...
        return this.precanvas.childNodes;

    }


    updateNodeLabelFromId(id, str,pre) {

        const content = this.container.querySelector('#node-' + id);
        const content2 = this.container.querySelector('#node-' + pre);

        var elems2 = content.querySelector('.conut');
        var el = content.querySelector('.statuss');


        var elemspre = content2.querySelector('.conut');


       /* elems2.forEach(value => {
            console.log(value)
            if ("LABEL" === value.tagName) {
            } else {
                value.value = elemspre.textContent + "-" +str
            }
        ...
        elems2.textContent = elemspre.textContent + "-" +str
        el.value = elemspre.textContent + "-" +str
        const inp = content.querySelector(".input_1")
        inp.style.backgroundColor = arrayColor[str - 1];

    }

    getNodeLabelFromId(id, i) {


        const content = this.container.querySelector('#node-' + id);
        var elems = content.querySelector('.conut');
    ...
    setNodeInputDfFromId(id, str, pre) {
        ...
        var elems = content.querySelectorAll('.statuss');
        var elemspre = content2.querySelector('.conut');


        for (var i = 0; i < elems.length; i++) {

            Array.from(elems[i].attributes).map((v, f) => {

                if (v.name.indexOf("df-") === 0) {

                    if (v.name.includes("status")) {
                        console.log(v)

                        elems[i].value = elemspre.textContent + "-" +str
                        this.drawflow.drawflow[moduleName].data[id].data["status"] = elemspre.textContent + "-" +str

                    } else {
                       // elems[i].value = ""
                       // this.drawflow.drawflow[moduleName].data[id].data[elemspre.textContent + "-" +str] = ""
    ...
    updateNodeDataFromId(id, data) {
        ...
        if (this.module === moduleName) {
            ...
            Object.entries(data).forEach(function (key, value) {
                if (typeof key[1] === "object") {
                   this.insertObjectkeyss(null, key[0], key[0]);
                } else {
                    var elems = content.querySelectorAll('[df-' + key[0] + ']');
                    for (var i = 0; i < elems.length; i++) {
                        if ("LABEL" === elems[i].tagName) {
...
nodeOption(editor, 1)
nodeOption2(editor, 1)
nodeOption3(editor, 1)

for (let i = 0; i < 30; i++) {
    nodeLista(editor,1,i)
}

nodeText(editor,1)
nodeML(editor,1).then()
nodeRedirect(editor)
nodeFile(editor)
...
editor.on("nodeRemoved", function (id) {
    nodesContrution.forEach((v,i) => {
        if(v.id === parseInt(id)){
            nodesContrution.splice(i, 1);
        }
    })

})

editor.on("nodeCreatedImport", function (id) {

    nodesContrution.push(NodeUI.instancie(editor,id))
...
editor.on("connectionCreated", function (id) {
    ...
    const f = parseInt(id.output_class.replace(/[^0-9]/g, ''))

   editor.setNodeInputDfFromId(id.input_id, f, id.output_id)
    editor.updateNodeLabelFromId(id.input_id, f, id.output_id)



    const nodeInfo = editor.getNodeFromId(id.input_id);
    if(nodeInfo.inputs[id.input_class].connections.length > 1) {
        const removeConnectionInfo = nodeInfo.inputs[id.input_class].connections[0];
        editor.removeSingleConnection(removeConnectionInfo.node, id.input_id, removeConnectionInfo.input, id.input_class);
    }

    const info = id
    const nodeInfo2 = editor.getNodeFromId(info.output_id);
    if(nodeInfo2.outputs[info.output_class].connections.length > 1) {
        const removeConnectionInfo2 = nodeInfo2.outputs[info.output_class].connections[0];
        editor.removeSingleConnection(info.output_id, removeConnectionInfo2.node, info.output_class, removeConnectionInfo2.output);
    }
})
...
editor.on("connectionRemoved", function (id) {
    ...
    const content = editor.container.querySelector('#node-' + id.input_id);
    const inp = content.querySelector(".input_1")
...
function export2() {
    ...
    nodesContrution.forEach(value => {

        console.log(value)

        value.build(exportvalue)
    })
...
function importaConversa(value) {
    editor.import(value);
}
...
//var acaomoda = document.getElementById("acaomodel");
/*
var span = document.getElementsByClassName("close")[0];
span.onclick = function () {
   // moda.style.display = "none";
    // acaomoda.style.display = "none";
}*/
...
function newAction() {
    ...
    const acaonode = document.createElement("div");
    const op = document.getElementById("mySelect").value

    acaonode.innerHTML = "<center><label>Ação</label> " +
        "<br/> <i class='fa fa-2x fa-" + op + "'  style='font-size: 80px ;width: 80px;height: 80px'  ></i> <br/> " +
        "</center>   "
    editor.registerNode("acaonode", acaonode);

    editor.addNode('github', 1, 0, 100, 250, 'github', data, "acaonode", true);

    acaomoda.style.display = "none";
}

function newResponta() {
    editor.addNode('github', 1, 0, 100, 250, 'github', data, "resposta", true);
}
...
const menu = new ContextMenu(
    {
        ...
        'items': [
            {
                'icon': 'envelope', 'name': 'Nova Pergunta', action: () => {
                    export2()
                }
            },
            {'icon': 'envelope', 'name': 'Nova Pergunta com Opções', action: ()=>{}},
            {'icon': 'download', 'name': 'Nova Resposta', action: editor.refresh()},
            {
                'icon': 'trash', 'name': 'Nove Erro', action: () => {
                    importexport()
                }
            },
            {
                'icon': 'trash', 'name': 'Nove Ação', action: () => {
                    acaomoda.style.display = "block";
                }
            },
        ]
    }
...
var namecli = document.createElement("div")
namecli.innerHTML = "<b>Nome do cliente aqui</b>"
namecli.classList.add("namecli")
//example.appendChild(namecli)
namecli.addEventListener("hover", (e) => {
